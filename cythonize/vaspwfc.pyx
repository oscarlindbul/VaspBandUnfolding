...
def expand_wfc(phi_k, grid_k):
	cdef int ii,jj,kk, ii_inv, jj_inv, kk_inv
	cdef cnp.ndarray[cnp.complex128_t, ndim=3] phi
	cdef cnp.ndarray[cnp.int_t, ndim=1] grid
	phi = phi_k
	grid = grid_k
...
class vaspwfc(object):
	...
	def wfc_r(self, ispin=1, ikpt=1, iband=1,
		...
		if ngrid is None:
			grid = self._ngrid.copy() * 2
		else:
			grid = np.array(ngrid, dtype=int)
			assert ngrid.shape == (3,)
			assert np.alltrue(grid >= self._ngrid), \
		...
		# \sum_{ijk} | \phi_{ijk} | ^ 2 = 1
		normFac = rescale if rescale is not None else np.sqrt(np.prod(grid))
		...
		if self._lgam:
			if self._gam_half == 'z':
				phi_k = np.zeros(
					(grid[0], grid[1], grid[2]//2 + 1), dtype=np.complex128)
			else:
				phi_k = np.zeros(
					(grid[0]//2 + 1, grid[1], grid[2]), dtype=np.complex128)
		else:
			phi_k = np.zeros(grid, dtype=np.complex128)

		gvec %= grid[np.newaxis, :]
		...
		if self._lsoc:
			...
			if self._lgam:
				...
				if self._gam_half == 'z':
					phi_k = expand_wfc(phi_k, np.swapaxes(grid, 0, 2))
...
#					phi_k[0, 0, 0] *= np.sqrt(2.)
					tmp_grid = grid
					temp_phi_k = phi_k
					tmp = np.fft.irfftn(temp_phi_k, s=tmp_grid) * normFac
				elif self._gam_half == 'x':
					phi_k = expand_wfc(phi_k, grid)
...
#					phi_k[0, 0, 0] *= np.sqrt(2.)
					...
					print("Calc ifft")
					tmp_phi_k = phi_k
					tmp_grid = (grid[2], grid[1], grid[0])
					tmp = np.fft.irfftn(
						tmp_phi_k, s=tmp_grid) * normFac
					print("ifft done")
...
#				if self._gam_half == 'x':
#					phi_np.swapaxes(phi_k, 0, 2)
#				tmp = np.fft.irfftn(phi_k, s=ordered_grid) * normFac
#				return tmp
			else:
	...
	def readBandCoeff(self, ispin=1, ikpt=1, iband=1, norm=False):
		...
		'''
		cdef long rec, nplw, i
		cdef cnp.ndarray[cnp.complex128_t, ndim=1] cg
		cdef double cg_norm
		...
		if norm:
			...
			for i in range(cg.shape[0]):
				cg_norm += cg[i].real**2 + cg[i].imag**2
			cg_norm = math.sqrt(cg_norm)
			cg = cg / cg_norm
		return cg
